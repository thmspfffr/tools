...
for j=1:nep;
    ...
    % scan data for nans and exclude segments (only 1 channel)
    for i=1:nseg
      tmp=dataep((i-1)*segshift+1:(i-1)*segshift+segleng,1);
      nanidx(i) = any(isnan(tmp));
    end
    idx = find(~nanidx);
    for i=1:length(idx) %average over all segments;
        dataloc=dataep((idx(i)-1)*segshift+1:(idx(i)-1)*segshift+segleng,:);
