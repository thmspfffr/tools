...
function resout=tp_data2orthopowcov_wavelet(data,f,filt)
...
for i=1:1
  ...
  res3=res3/kk;
  %
  resout=(res1-res2.*res3);
  resout=(resout+resout')./2;
  
  % aal 6mm?
  % reduce to 90 regions
  if size(resout,1)>5000
    reg_idx = 1:90;
    fprintf('Combining atlas regions ...\n')
    for ireg = reg_idx
      ireg
      idx{ireg} = find(filt.sa.aal_label==ireg);
