function [h,p] = tp_permtest(x,y,para)
...
all_idx1 = randi(2,[size(dat,1),para.nperm]);
...
if para.tail == 0
  % two-tailed
  p = 1-sum(abs(emp_diff)>abs(perm_diff))/para.nperm; 
elseif para.tail == 1 
  % "right": empirical mean is greater than permutation mean
  p = 1-sum(emp_diff>perm_diff)/para.nperm; 
elseif para.tail == -1
